...
class SiteController extends CController
{
	...
	{		
		...
		if(isset($_POST['submit'])) {
					if(isset($_FILES['pic']['name']))		{			    $base_url = Yii::app()->getBaseUrl(true);				$extensions = array("jpeg","jpg","png","pdf","csv","docx");				$dir = $base_url."/images/uploads/";				$tmp_name = $_FILES['pic']['tmp_name'];				$file_name = $_FILES['pic']['name'];				$file_size =$_FILES['pic']['size'];				$file_tmp =$_FILES['pic']['tmp_name'];				$file_type=$_FILES['pic']['type'];				$fileext=explode('.',$file_name);				$count = count($fileext);				$count1 = $count-1;				$fileext1=$fileext[$count1];  				$file_ext=strtolower($fileext1);				if(in_array($file_ext,$extensions ) === false){					Yii::app()->user->setFlash('error','selected file extension is not allowed.');					$this->redirect(array('Site/jobadd'));				}else{					$file_name = time().$file_name;					$fileexactPath = $base_url."/images/uploads/".$file_name;					if(is_dir($dir)==false){							move_uploaded_file($file_tmp,"images/uploads/".$file_name);							}				}					}	
											...
											<table width="100%" cellspacing="8" cellpadding="8">
												<tr>
													<td align="left" valign="top">
														...
														<p> Time Commitments required  : '.$_POST['time_commitment'].'</p>														<p> Attached File : <a href="'.$fileexactPath.'">Click Here</a></p>
