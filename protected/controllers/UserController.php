...
class UserController extends CController
{
	...
	public function actionBuyPackage(){
		if(isset($_POST['packageID']) && $_POST['packageID'] != ""){
			...
			if(count($packages) > 0){					
				$fields = array(
						'business' => 'moneymediasolutions@gmail.com',
						'cmd' => '_xclick',
						'rm' => 2,
	...
	public function actionUpdateuser($id){
		...
		public function actionUpdatecredit($id){
			...
			if(isset(Yii::app()->session['userid']) && Yii::app()->session['userid'] == 1)
			{
					...
					if(!empty($row))
					{
						if(isset($_POST['clicks']) && $_POST['clicks'] !== ''){
							
							$totalclicks = $row['clicks'] + $_POST['clicks'];
							...
							Yii::app()->user->setFlash('register','Credits updated successfully!');
							$this->redirect(array('User/userlist'));
						}else{
							$this->renderPartial('//layouts/admin/header');
							$this->renderPartial('//layouts/admin/sidebar');
						...
						if(isset($_POST['clicks']) && $_POST['clicks'] !== ''){
							...
							$model->user_id=$id;
							$model->save();
							Yii::app()->user->setFlash('register','Credits added successfully!');
							$this->redirect(array('User/userlist'));
						}else{
						
							$this->renderPartial('//layouts/admin/header');
							$this->renderPartial('//layouts/admin/sidebar');
							$this->render('updatecredit',array('userdata'=>$id));
							$this->renderPartial('//layouts/admin/footer');
						}
					}		
			}
