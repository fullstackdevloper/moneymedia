...
class AdminController extends CController
{
	...
	{				if(isset(Yii::app()->session['userid']) && Yii::app()->session['userid'] == 1){			$this->redirect(array('Admin/dashboard'));		}else{						$model=new LoginForm('Back');			if(isset($_POST['LoginForm']))			{							$model->attributes=$_POST['LoginForm'];				if($model->validate() && $model->login())				{					Yii::app()->user->setFlash('login','Successfully logged in');					$this->redirect(array('Admin/dashboard'));				}				else				{					Yii::app()->user->setFlash('error','Invalid username or password');				}			}else{						$this->render('login',array('model'=>$model));			}		} 
	}
