...
class CommonFnc extends CApplicationComponent {
 ...
 public function checkPackageActive($userid = null){
	...
	$getPackageCountsByadmin = Yii::app()->db->createCommand()
			 ->select('clicks')
			 ->from('cads_user_clicks_by_admin ')
			 ->where('user_id =:userid', array(':userid' => $userid))
			 ->queryAll();
	
	
	
	if(!empty($getPackageCountsByadmin)){
		foreach($getPackageCountsByadmin as $packcount){
				$countSet += $packcount['clicks'];
			}
	}	
